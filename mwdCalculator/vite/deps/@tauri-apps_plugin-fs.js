import { n as Resource, r as invoke, t as Channel } from "./core-BMLXHDsj.js";
...
async function create(path, options) {
	...
	return new FileHandle(await invoke("plugin:fs|create", {
...
async function copyFile(fromPath, toPath, options) {
	if (fromPath instanceof URL && fromPath.protocol !== "file:" || toPath instanceof URL && toPath.protocol !== "file:") throw new TypeError("Must be a file URL.");
...
async function readTextFileLines(path, options) {
	...
	return await Promise.resolve({
		...
		async next() {
			...
			return {
				value: decoder.decode(bytes.slice(0, bytes.byteLength - 1)),
				done
			};
		},
...
async function writeTextFile(path, data, options) {
	if (path instanceof URL && path.protocol !== "file:") throw new TypeError("Must be a file URL.");
	await invoke("plugin:fs|write_text_file", new TextEncoder().encode(data), { headers: {
		path: encodeURIComponent(path instanceof URL ? path.toString() : path),
		options: JSON.stringify(options)
	} });
}
...
async function watchInternal(paths, cb, options) {
	...
	for (const path of watchPaths) if (path instanceof URL && path.protocol !== "file:") throw new TypeError("Must be a file URL.");
	const onEvent = new Channel();
	onEvent.onmessage = cb;
	const watcher = new Watcher(await invoke("plugin:fs|watch", {
...
async function size(path) {
	if (path instanceof URL && path.protocol !== "file:") throw new TypeError("Must be a file URL.");
	return await invoke("plugin:fs|size", { path: path instanceof URL ? path.toString() : path });
}
/**
...
async function startAccessingSecurityScopedResource(path) {
	if (path instanceof URL && path.protocol !== "file:") throw new TypeError("Must be a file URL.");
	await invoke("plugin:fs|start_accessing_security_scoped_resource", { path: path instanceof URL ? path.toString() : path });
}
/**
...
async function stopAccessingSecurityScopedResource(path) {
	if (path instanceof URL && path.protocol !== "file:") throw new TypeError("Must be a file URL.");
	await invoke("plugin:fs|stop_accessing_security_scoped_resource", { path: path instanceof URL ? path.toString() : path });
}
//#endregion
export { BaseDirectory, FileHandle, SeekMode, copyFile, create, exists, lstat, mkdir, open, readDir, readFile, readTextFile, readTextFileLines, remove, rename, size, startAccessingSecurityScopedResource, stat, stopAccessingSecurityScopedResource, truncate, watch, watchImmediate, writeFile, writeTextFile };
